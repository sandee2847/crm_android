...
const userRegister = async (req, res) => {
    try {
        ...
        if (!name || !email || !mobile || !password || !confirmPassword || !isValidRequest(newUser)) {
        ...
        confirmPassword = await bcrypt.hashSync(newUser.confirmPassword, 10)
        ...
        res.status(201).send({ status: true, message: "Register Successfully" })
...
const getEmpDetails = async(req,res)=>{
    try {
     ...
     let empDetails = await Users.findOne({employeeId:empId ,isDeleted:disable }).select({name:1, email:1, mobile:1})
     if(!empDetails){
         return res.status(404).send({status:false, message:'Data not found'})
     }
     return res.status(200).send({status:true, data:empDetails})
    } catch (error) {
     return res.status(500).send({status:false, message:error.message})
    }
 }
 // make unable and disable employee
 const removeEmp = async(req, res)=>{
     ...
     if(status == 'disable' || status == 'Disable'){
      removedEmp = await Users.findOneAndUpdate({employeeId:empId}, {isDeleted:"disable"},{new:true})
     }
     removedEmp = await Users.findOneAndUpdate({employeeId:empId}, {isDeleted:'unable'}, {new:true})
     if(!removedEmp){
 ...
 //get all leads of specific employee
const leadsDetails = async(req,res)=>{
    try {
     ...
     let fetchedLeads = await Leads.find({employeeId:empId})
     if(fetchedLeads.length ==0){
         return res.status(404).send({status:false, message:"Leads not found"})
     }
     return res.status(200).send({status:true, leads:fetchedLeads})
    } catch (error) {
     return res.status(500).send({status:false, message:error.message})
    }
 }
...
module.exports = {
    ...
    , varifyOtp, resetPassword, getAllUserId, getEmpDetails, removeEmp, leadsDetails
};