...
export default function Allocated({ navigation }) {
  ...
  const getCookies = async () => {
    try {
      ...
      setCookies(cookies);
      fetch("http://162.240.226.166:80/employee/leads/Allocated", {
        method: 'GET',
        headers: {
  ...
  const getWork = email => {
    navigation.navigate("singleLead", { email: email });
  };
  ...
  const acceptLead = async () => {
    try {
      ...
      fetch("http://162.240.226.166:80/accept/lead", {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${cookies.mycookies.value}`,
        },
  ...
  return (
    <SafeAreaView >
      <ScrollView>
        ...
        <Button title="Accept" onPress={() => acceptLead()} />
      </ScrollView>
    </SafeAreaView>
  );
}
