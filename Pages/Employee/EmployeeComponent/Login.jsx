...
const Login = ({ navigation }) => {
  ...
  console.log("token value... Satya Mobile", token);
  const getNotifyToken = async () => {
    try {
      let fcmToken = await AsyncStorage.getItem("fcmToken");
      console.log(fcmToken, "from get notify function")
      setToken(fcmToken);
    } catch (error) {
      console.log(error)
    }
  ...
  const Sendtobackend = () => {
    if (email == '' || password == '') {
      ...
      try {
        fetch('http://162.240.226.166:80/userLogin', {
          method: 'POST',
          headers: {
    ...
    try {
      fetch('http://162.240.226.166:80/add/notification/token', {
        method: 'POST',
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      <KeyboardAvoidingView behavior='height' style={styles.container1} >
        <View style={styles.s2}>
          ...
          <View style={styles.fp}>
            ...
            <Pressable style={{ marginTop: 10 }} onPress={() => navigation.navigate('Sign Up', { userId: "Sign_Up" })}>
